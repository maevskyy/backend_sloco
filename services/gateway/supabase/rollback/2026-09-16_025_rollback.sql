-- Undo for migration 025 (SLO-5). Restores public.feed_fallback_places to the
-- migration-021 body (dev branch, live on prod since 2026-08-13), copied verbatim.
--
-- Idempotent: CREATE OR REPLACE, same six-argument signature. The expression
-- index places_city_norm_idx is left in place on purpose (rollbacks do not drop
-- schema; it is harmless and the 021 body cannot use it anyway). After running it
-- the fallback feed is slow again (whole-catalog scan + photo join before LIMIT)
-- — run only to undo 025.
