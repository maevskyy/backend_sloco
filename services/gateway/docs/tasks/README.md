...
| In progress | `TASKS_45_SEARCH_CATEGORY_RADIUS.md` | iOS ask: `category` (seven-bucket vocabulary in `places/common/place-buckets.ts`, word-boundary matched) + `radiusMeters` hard filter on `/v1/search/places`; browse mode with optional `q`. Implemented on `dev`; awaiting migration `018` + deploy. |
| In progress | `TASKS_46_FEED_CATEGORY_FILTER.md` | iOS ask: `category` on `/v1/feed/places` — RPC-level filter on the fallback path (migration `019`), post-hydration filter + positional re-rank on the personalized path. Implemented on `dev`; awaiting migrations + deploy. |
