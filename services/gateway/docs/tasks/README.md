...
| Done in code | `TASKS_51_EVENT_LOG.md` | Event log MVP (x-algorithm spec): `POST /v1/events` append-only intake (dedupe by event_id, unknown types kept, 429 limits), `identity_links` stitching, serving receipts `rec_served`+`rec_served_items` written async by the feed (requestId/position now on the feed response), action-weights JSON config `v1_2026-08`, nightly parquet export script. Migration `022`. Rec half: rec `TASKS_8`. Awaiting prod migration + deploy + cron. |
