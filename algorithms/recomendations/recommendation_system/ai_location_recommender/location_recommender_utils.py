...
def recommend_for_profile_clusters(
    ...
    candidates = df_candidates.drop(index=favorites.index, errors="ignore").copy()
    if candidates.empty:
        return pd.DataFrame()

    candidate_vectors = normalize(_matrix_for_rows(candidates, embedding_matrix))
    profile_groups = list(favorites.groupby("profile_cluster"))
    if top_n_per_cluster is None:
        top_n_per_cluster = int(np.ceil(total_n / max(len(profile_groups), 1)))

    recommendations = []
    for profile_cluster, fav_group in profile_groups:
        centroid = normalize(_matrix_for_rows(fav_group, embedding_matrix).mean(axis=0, keepdims=True))
        similarity = cosine_similarity(candidate_vectors, centroid).ravel()
        similarity_norm = np.clip((similarity + 1) / 2, 0, 1)
        scored = candidates.copy()
        scored["similarity"] = similarity
        scored["similarity_norm"] = similarity_norm
        scored["profile_cluster"] = int(profile_cluster)
        scored["based_on_favorites"] = "; ".join(fav_group["name"].astype(str).head(8))

        if "quality_score" not in scored.columns:
            scored["quality_score"] = compute_quality_score(scored)
        scored["tag_overlap"] = tag_overlap_score(scored, fav_group)
        scored["axis_similarity"] = axis_similarity(scored, fav_group)
        scored["price_match"] = price_match_score(scored, fav_group)

        if mode == "pure":
            scored["recommendation_score"] = scored["similarity"]
            sort_col = "similarity"
        elif mode == "hybrid":
            scored["recommendation_score"] = (
                scored["similarity_norm"] * weights["semantic_similarity"]
                + scored["tag_overlap"] * weights["tag_overlap"]
                + scored["axis_similarity"] * weights["axis_similarity"]
                + scored["quality_score"] * weights["quality_score"]
                + scored["price_match"] * weights["price_match"]
            )
            sort_col = "recommendation_score"
        else:
            raise ValueError("mode must be 'pure' or 'hybrid'")

        recommendations.append(scored.sort_values(sort_col, ascending=False).head(top_n_per_cluster))

    result = pd.concat(recommendations).sort_values("recommendation_score", ascending=False)
    result = result[~result["place_id"].duplicated(keep="first")].head(total_n).copy()
