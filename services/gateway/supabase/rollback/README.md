...
- `2026-08-28_023_system_lists_rollback.sql` — undoes `023` (system saved lists).
  **Breaks the rules above:** it deletes the two system collections with their
  membership rows and drops the `slug` column. Kept as written on `dev` because it
  is what exists; rewriting it to the rules is SLO-16. Read its header before use.
- `2026-09-16_024_rollback.sql` — undoes `024` (`feed_places_by_source_ids` joins on
  `(source, source_id)`). Restores the migration-`016` body.
- `2026-09-16_025_rollback.sql` — undoes `025` **and** `026` (`feed_fallback_places`
  city cut through the covering index, hydrate after LIMIT). Restores the
  migration-`021` body; the indexes stay.
- `2026-09-16_027_rollback.sql` — undoes `027` (`search_places` city cut, index-only
