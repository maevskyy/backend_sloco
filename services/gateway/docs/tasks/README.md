...
| Done | `TASKS_38_ONBOARDING_COMPLETE.md` | Onboarding 1/5 (GW): new `onboarding` module + `POST /v1/onboarding/complete` — picks → `saved_places` (favourites), status upsert on `profiles`, `/v1/me` enum documented. Live: write→read verified, and the same user's next feed came back `personalized`. |
...
| Done | `TASKS_41_MAP_TILE_DENSITY_CAP.md` | iOS ask: per-tile top-N cap in `map_tile()` via per-zoom `LIMIT` (6/10/15/25/uncapped), score floor kept only ≥z18; `MAP_TILE_VERSION` bump busts caches. One migration + one env. |
| Done | `TASKS_42_FEED_SORT.md` | iOS ask: `sort=relevance\|distance` on `/v1/feed/places` — app-layer stable re-sort of the snapshot (RPCs already return `distance_m`), positional rank, 400 validation, `feed.sort` echo. No migration. |
| Done | `TASKS_43_FEED_SNAPSHOT_DEPTH.md` | iOS ask: raise `FEED_SNAPSHOT_SIZE` 100→200 (RPC caps and rec-service limits already allow 200 since `TASKS_37`/migration `016`). One-line change + tests. |
| Done | `TASKS_44_PLACE_DATA_BACKFILLS.md` | iOS ask (quick half): NULL-guarded backfills — `price_level` restored from `places.raw` (the mapper had parsed the catalog's categorical words as numbers → NULL everywhere) and `google_maps_uri` synthesized from the CID; both mapper bugs fixed. Data ops, no migration. |
| Done | `TASKS_45_SEARCH_CATEGORY_RADIUS.md` | iOS ask: `category` (seven-bucket vocabulary in `places/common/place-buckets.ts`, word-boundary matched) + `radiusMeters` hard filter on `/v1/search/places`; browse mode with optional `q`. Live: 20 cafés within 1.5 km, zero other-city results, browse latency 0.19 s. `nature`/`shopping` are near-empty in the current catalog. |
| Done | `TASKS_46_FEED_CATEGORY_FILTER.md` | iOS ask: `category` on `/v1/feed/places` — RPC-level filter on the fallback path (migration `019`), post-hydration filter + positional re-rank on the personalized path. Live: a chip returns 50 cards + 50 more at `offset=50`, composes with `sort=distance`. |
...
| Done | `TASKS_51_EVENT_LOG.md` | Event log MVP (x-algorithm spec): `POST /v1/events` append-only intake (dedupe by event_id, unknown types kept, 429 limits), `identity_links` stitching, serving receipts `rec_served`+`rec_served_items` written async by the feed (requestId/position now on the feed response), action-weights JSON config `v1_2026-08`, nightly parquet export script. Migration `022`. Rec half: rec `TASKS_8`. Live 2026-08-16: full acceptance incl. the first labeled training row (impression+open+dwell+save joined to a real serving). |
| Done | `TASKS_52_EVENT_LOG_RETENTION.md` | Event-log retention: `--retention-days N` on the export script deletes exported days older than N (per-day, transactional, only days whose parquet files exist — missing export = day kept and reported; `identity_links` never touched). Caps DB growth at ~N×daily volume. Live 2026-08-16: cron runs with `--retention-days 30`; verified end to end (window still empty). |
| Done in code | `TASKS_53_FAVORITES_DEFAULT_LIST.md` | The auto-created default collection is renamed `Want to go` → `Favorites` (one constant; the list was already auto-created, hidden from My lists and undeletable). iOS Profile → Favorites now opens it. Recommender weighting deliberately unchanged — see the file's open product question. Needs the one-off rename SQL for existing users. |
| Done in code | `TASKS_54_SYSTEM_SAVED_LISTS.md` | Save now ASKS which lists (multi-select picker). Three system lists — Saved / Favorites / Been there — via a new `slug` column (migration `023`): auto-created, undeletable, hidden from My lists. New `PUT /v1/me/saved/places/:id/collections` sets membership exactly (empty = unsave); unsave now clears membership. Recommender: Saved = want_to_go, Favorites = favourites, Been there = no signal. iOS: picker sheet + Been there screen (Figma 356:3160/356:3523). Needs migration 023 + deploy. |
