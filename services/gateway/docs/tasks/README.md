...
| Done in code | `TASKS_52_EVENT_LOG_RETENTION.md` | Event-log retention: `--retention-days N` on the export script deletes exported days older than N (per-day, transactional, only days whose parquet files exist — missing export = day kept and reported; `identity_links` never touched). Caps DB growth at ~N×daily volume. Activate by adding `--retention-days 30` to the cron line. |
