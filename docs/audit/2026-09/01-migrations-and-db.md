...
**Перенос в `main` (2026-09-16):** файлы 017–023, их rollback-скрипты и `rollback/README.md` скопированы из `dev` в дерево `main` байт-в-байт — первая тема переноса dev → main. Дерево теперь отражает живую схему (001–027). Не перенесены `supabase/01_load_places_staging.sql` / `02_merge_places.sql` — это скрипты импорта каталога, пойдут вместе с темой `places`/импорт.
