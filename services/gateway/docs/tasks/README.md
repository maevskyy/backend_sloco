...
| In progress | `TASKS_38_ONBOARDING_COMPLETE.md` | Onboarding 1/5 (GW): new `onboarding` module + `POST /v1/onboarding/complete` — picks → `saved_places` (favourites), status upsert on `profiles`, `/v1/me` enum documented. Implemented on `dev`; awaiting deploy + live verify. |
